  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-6 py-16">
      ...
      <section className="mb-16">
        <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-purple-500/40 bg-purple-100 px-3 py-1 text-xs font-medium text-purple-700 dark:bg-purple-500/10 dark:text-purple-300">
          <span className="h-1.5 w-1.5 rounded-full bg-purple-400" />
          B2A · Infrastructure for autonomous AI
        </div>
        <h1 className="mb-4 text-5xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-100 sm:text-6xl">
          AgentGate
        </h1>
        <p className="mb-6 max-w-2xl text-xl text-zinc-700 dark:text-zinc-300">
        ...
        <p className="mt-6 max-w-2xl rounded-lg border-l-2 border-red-500/60 bg-red-50 px-4 py-3 text-sm text-zinc-700 dark:bg-red-500/5 dark:text-zinc-300">
          <span className="font-semibold text-red-700 dark:text-red-300">In July 2025</span>,
      ...
      {/* HOW IT WORKS */}
      <section className="mb-16">
        <h2 className="mb-6 text-2xl font-semibold text-zinc-900 dark:text-zinc-100">How it works</h2>
        <pre className="overflow-x-auto rounded-lg border border-zinc-200 bg-zinc-50 p-5 font-mono text-xs leading-relaxed text-zinc-700 dark:border-zinc-800 dark:bg-zinc-900/60 dark:text-zinc-300">
{`  ┌──────────┐       ┌─────────────┐       ┌──────────┐
  │          │       │             │       │  HUMAN   │
  ...
  └──────────┘       └─────────────┘       └──────────┘
      ...
      <section className="mb-16">
        ...
        <div className="flex flex-wrap gap-2 text-xs">
          {[
            ...
            <span
              key={t}
              className="rounded-full border border-zinc-200 bg-zinc-50 px-3 py-1 text-zinc-700 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-300"
            >
              {t}
            </span>
          ))}
      ...
      {/* CTA */}
      <section className="rounded-lg border border-emerald-500/40 bg-emerald-50 p-6 dark:bg-emerald-500/5">
...
function Stat({
  ...
  const tone = {
    zinc: "text-zinc-900 dark:text-zinc-100 border-zinc-200 dark:border-zinc-800",
    amber: "text-amber-600 dark:text-amber-300 border-amber-500/40",
    purple: "text-purple-700 dark:text-purple-300 border-purple-500/40",
    red: "text-red-600 dark:text-red-300 border-red-500/40",
    emerald: "text-emerald-600 dark:text-emerald-300 border-emerald-500/40",
  }[accent];
  // Split text classes (first 2 tokens) from border classes (last 2 tokens).
  const parts = tone.split(" ");
  const textCls = parts.slice(0, 2).join(" ");
  const borderCls = parts.slice(2).join(" ");
  return (
    <div className={`rounded-lg border bg-white dark:bg-zinc-900/50 p-4 ${borderCls}`}>
      <div className="text-xs uppercase tracking-wide text-zinc-500">
        {label}
      </div>
      <div className={`mt-1 text-3xl font-semibold ${textCls}`}>
        {value}
      </div>
    </div>
  );
...
function Layer({
  ...
  const tone = {
    purple: "border-purple-500/40 bg-purple-50 dark:bg-purple-500/5 text-purple-700 dark:text-purple-300",
    amber: "border-amber-500/40 bg-amber-50 dark:bg-amber-500/5 text-amber-700 dark:text-amber-300",
    emerald: "border-emerald-500/40 bg-emerald-50 dark:bg-emerald-500/5 text-emerald-700 dark:text-emerald-300",
  }[accent];
  return (
    <div className={`rounded-lg border p-5 ${tone}`}>
      <div className="mb-2 text-2xl font-bold">{num}</div>
      <div className="mb-2 text-base font-semibold text-zinc-900 dark:text-zinc-100">{title}</div>
...
function Demo({
  ...
  return (
    <div className="rounded-lg border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900/50 p-4">
      <div className="mb-1 font-mono text-xs text-emerald-700 dark:text-emerald-300">{cmd}</div>
      <div className="text-base font-medium text-zinc-900 dark:text-zinc-100">{title}</div>
